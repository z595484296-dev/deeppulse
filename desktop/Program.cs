    ...
    private async Task<Uri?> ProbeDeepPulseAsync(int port, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!health.TryGetProperty("capabilities", out var capabilities)
                ...
                || !capabilities.TryGetProperty("ai_research_duty", out var aiResearchDuty)
                || aiResearchDuty.ValueKind != JsonValueKind.Number
                || !aiResearchDuty.TryGetInt32(out var aiResearchDutyVersion)
                || aiResearchDutyVersion != 1
                || !capabilities.TryGetProperty("ai_research_job_receipts", out var aiResearchJobReceipts)
                || aiResearchJobReceipts.ValueKind != JsonValueKind.Number
                || !aiResearchJobReceipts.TryGetInt32(out var aiResearchJobReceiptsVersion)
                || aiResearchJobReceiptsVersion != 1
                || !capabilities.TryGetProperty("ai_research_budget", out var aiResearchBudget)
