...
import { state, marketState, bus, emit, loadAlerts, markTriggered, syncProfile } from './store.js?v=1.42.1';
import { esc, fmtPct, fmtPrice, pctClass, tradingState, toast } from './util.js?v=1.42.1';

import * as pageOverview from './pages/overview.js?v=1.42.1';
import * as pageEmotion from './pages/emotion.js?v=1.42.1';
import * as pageMarket from './pages/market.js?v=1.42.1';
import * as pageLadder from './pages/ladder.js?v=1.42.1';
import * as pageWatch from './pages/watch.js?v=1.42.1';
import * as pageStrategy from './pages/strategy.js?v=1.42.1';
import * as pageEpaper from './pages/epaper.js?v=1.42.1';
import * as pageDatasrc from './pages/datasrc.js?v=1.42.1';
import * as pageAbout from './pages/about.js?v=1.42.1';
import { createChatView, chatStore, ensureGreeting } from './chat.js?v=1.42.1';
import { EMBEDDED, initBridge, exitToSession, applyTheme, askDeepSeek, setBridgeContextProvider } from './bridge.js?v=1.42.1';
import { initOnboarding } from './onboarding.js?v=1.42.1';
import { attentionContext, initAttentionCenter, publishAttention } from './attention-center.js?v=1.42.1';
import { initAiProviderUi } from './ai-provider-ui.js?v=1.42.1';
...
async function navigateAttentionTarget(target, group) {
  ...
  if (target.entityType === 'observation_rule') {
    return requestTargetFocus('observation-rule-open', { ...detail, ruleId: target.entityId });
  }
  ...
  return {
    ...
    researchWorkflows: state.researchWorkflows ? {
      ...
      items: (state.researchWorkflows.items || []).slice(0, 12).map(row => ({
        ...
        watch: row.watch ? {
          modelVersion: row.watch.modelVersion, enabled: row.watch.enabled === true,
          status: row.watch.effectiveStatus || row.watch.status,
          sources: row.watch.sources, frequency: row.watch.frequency,
          delivery: row.watch.delivery, startedAt: row.watch.startedAt,
          expiresAt: row.watch.expiresAt, nextCheckAt: row.watch.nextCheckAt,
          lastCheckedAt: row.watch.lastCheckedAt, lastChangeAt: row.watch.lastChangeAt,
          pausedSources: row.watch.pausedSources, lastChange: row.watch.lastChange,
          boundary: row.watch.boundary,
        } : null,
        contract: row.contract,
      })),
      watchSummary: state.researchWorkflows.watchSummary,
